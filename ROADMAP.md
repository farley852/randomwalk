...
- `vitest` を devDependencies に追加（現状 `devDependencies` セクション自体が未作成）
- 対象（優先順）:
...
- ドラッグでパン
- ダブルクリックでフィットリセット
- **注意**: 現在の `camera.ts` は全ポイントからビューポートを自動計算する設計。手動ズーム・パンの導入はカメラ制御の大幅な変更を伴うため、Phase 4 内で最も影響が大きい
...
- **テーマ切替**: 背景色 `#0f0f23` がハードコードされているため、ライトモード対応にはテーマシステムの導入が必要
- **アクセシビリティ**: キーボードナビゲーション、スクリーンリーダー対応、カラーコントラスト改善
- **PWA 対応**: Service Worker によるオフライン利用
